...
template <typename T>
struct FunctionTraits<T, typename std::is_member_function_pointer<T>::type >  // 成员函数匹配，注意和 数据成员的区别, 这里也可以用 std::enable_if 来实现
{
    ...
    {
    public:
        Binding(const BoundType& bound_type) : bound_type_(bound_type) {
        }
        Detail::ReturnType access(Detail::ClassType& c) const {
            return (c.*bound_type_)();
        }
