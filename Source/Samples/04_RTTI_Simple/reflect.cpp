...
std::ostream& operator << (std::ostream& cout, const TypeDescriptor& typeDescriptor) 
{
    ...
    for(auto item = typeDescriptor.member_vars_.begin(); item != typeDescriptor.member_vars_.end(); item++) {
        cout << "Membervariable name:" << item->name() << std::endl;
    };

    for(auto item = typeDescriptor.member_funcs_.begin(); item != typeDescriptor.member_funcs_.end(); item++) {
