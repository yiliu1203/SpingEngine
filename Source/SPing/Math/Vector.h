...
public:
    ...
    constexpr Vector(const std::initializer_list<T> & inilist): Vector(inilist.begin(), index_sequence_type{}) 
    {
        // 没法静态检查 inilist的size 是否符合， 这里只能运行时检查了
        assert(inilist.size() == size);
    };
    constexpr Vector(const_pointer p): Vector(p, index_sequence_type{}) {};
    Vector(const Vector& vec): Vector(vec.data_.begin(), index_sequence_type{}) {}; 
...
private:
    ...
    // constexpr Vector(const_pointer p, std::index_sequence<Index...>): data_({(Index, *p++)...}) {}
    template<size_t ...Index>
    Vector(const_iterator p, std::index_sequence<Index...>): data_({(Index, *p++)...}) {}
