...
public:
    ...
    {
        public:
            Handler(void *p, FProxy f): receiver_(p), fun_(f)
            {
            }
            ...
            void operator == (const Handler* rhs)
            {
                return (receiver_ == rhs->receiver_ && fun_ == rhs->fun_);
            }
    ...
    Delegate(const Handler& rhs) : handlers_(1, rhs)
    {
    }
